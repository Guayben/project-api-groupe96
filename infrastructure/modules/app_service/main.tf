...
resource "azurerm_linux_web_app" "shop_app_service" {
  ...
  app_settings = {
    API_KEY                 = var.api_key                 # Clé API utilisée par l'application.
    COSMOSDB_ENDPOINT       = var.cosmosdb_endpoint       # Endpoint du service CosmosDB.
    COSMOSDB_READONLY_KEY   = var.cosmosdb_readonly_key   # Clé de lecture seule pour CosmosDB.
    COSMOSDB_DATABASE_NAME  = var.cosmosdb_database_name  # Nom de la base de données CosmosDB.
    COSMOSDB_CONTAINER_NAME = var.cosmosdb_container_name # Nom du conteneur dans CosmosDB.
  }
